...
7. 수집된 근거를 긍정/부정/정책/생활 인프라/실거래가로 분류하고, 정비사업 관련 근거는 진행 단계(조합설립~착공/준공)에 따라 가중치를 다르게 반영해 0~100점 전망 점수를 냅니다.
8. `OPENAI_API_KEY`가 있으면 OpenAI Responses API로 분석 코멘트를 추가합니다.
