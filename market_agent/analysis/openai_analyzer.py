...
class OpenAIReportAnalyzer:
    ...
    def enhance(self, report: AnalysisReport) -> AnalysisReport:
        ...
        response = client.responses.create(
            ...
            text={"verbosity": "low"},
            instructions=(
                "너는 한국 부동산 입지·상권 분석가다. 보수적인 투자 검토 관점으로 판단한다. "
                "제공된 근거만 사용하고, 생활 인프라만으로 가격 상승을 단정하지 않는다. "
                "가격을 보장하지 말고, 투자 조언처럼 단정하지 않는다. "
                "학교·마트·병원 같은 생활 인프라(local_factors)는 이미 현재 시세에 반영된 "
                "'기존 기반'이지 '상승동력'이 아니다. 상승동력(good_news)은 재건축·재개발 "
                "이주 계획, 신규 교통망 발표, 대형 상업시설 입점 확정처럼 앞으로 시세에 "
                "새로 영향을 줄 변화 신호만 해당한다. good_news가 비어 있으면 생활 인프라 "
                "설명으로 대신 채우지 말고, 그런 변화 신호가 아직 확인되지 않았다고 명확히 밝힌다. "
                "한국어로 4문장 이내의 핵심 인사이트만 작성한다. "
                "형식: 1) 한줄 결론 2) 상승동력(없으면 '확인된 변화 신호 없음'이라고 명시) "
                "3) 주의변수 4) 추가확인."
            ),
