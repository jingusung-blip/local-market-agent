...
POSITIVE_KEYWORDS = {
    ...
    "확정",
    "유치",
    # 2026-08-10 실사용 미팅 피드백: "생활 인프라(이미 반영된 현재 상태)"와
    # "상승동력(앞으로의 변화)"을 구분해야 한다는 지적 이후, 앞으로의 변화를
    # 알리는 신호를 더 잘 잡아내기 위해 추가한 키워드들. 상무님이 예로 든
    # 시나리오(재건축 이주로 인한 전세가 급등, 인근 시설 이전으로 생기는
    # 부지에 신규 상업시설 입점)를 그대로 반영함.
    "이주",  # 재건축/재개발 이주는 REDEVELOPMENT_STAGE_WEIGHTS에도 있지만,
    # 그 가중치는 "재건축/재개발/정비사업/정비구역" 태그가 이미 있어야만
    # 적용된다. "이주"를 POSITIVE_KEYWORDS에도 넣어서, 기사 문구가 정비사업
    # 관련 다른 키워드 없이 "이주"만 언급해도 최소한 긍정 신호로는 잡히게 함.
    "이전",  # 대형 시설(운전면허시험장 등)이 다른 곳으로 이전하면 그 부지에
    # 새 개발이 들어올 가능성을 알리는 신호.
    "예타",  # 예비타당성조사 통과는 교통망 계획이 뉴스 카더라를 넘어 정부
    # 공식 절차에 들어갔다는 의미라 신뢰도 높은 전망 신호.
    "환승센터",
    "신설",  # "OO역 신설", "OO초등학교 신설"처럼 새로 생기는 시설.
}
...
# Real-estate blog/marketing pages commonly render their nav/category tabs
# as a flat "위치 | 분양가 | 평면 | 청약 | 미분양 | 모델하우스"-style list, and
# search snippets pull that raw text in. A "미분양" tab label just means the
# page HAS a section about vacancy status -- it says nothing about whether
# vacancy actually exists. Treating every stray keyword in a pipe-delimited
# tag list as an assertion produces false positives (e.g. a fully sold-out
# complex getting flagged for "미분양" because its own site menu lists that
# topic). If a snippet looks like a tag list rather than prose, keyword
# sentiment classification is skipped.
TAG_LIST_PIPE_THRESHOLD = 3


def is_tag_list_text(text: str) -> bool:
    return text.count("|") >= TAG_LIST_PIPE_THRESHOLD
...
def classify_sentiment(text: str, default: str = "neutral") -> tuple[str, list[str]]:
    ...
    if is_tag_list_text(text):
        # Keep policy tags (e.g. "공고") since that's a legitimate topic
        # label for grouping, but don't let stray menu items drive sentiment.
        return default, sorted(set(policies))
...
def estimate_impact(text: str, category: str, default_sentiment: str = "neutral") -> float:
    ...
    if is_tag_list_text(text):
        # Same reasoning as classify_sentiment: don't let a stray menu-label
        # keyword (e.g. "지연", "확정") add ad-hoc bonuses/penalties either.
        return 0.0
