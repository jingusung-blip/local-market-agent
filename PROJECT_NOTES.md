...
## 다음 세션에서 이어서 할 일 (2026-08-10 기준, 여기서부터 이어가면 됨)

**2026-08-10: 실사용 미팅 피드백 3건 반영 (신뢰도 버그 수정, 전세가율 추세 표시, 입주물량 조사)**

- 계기: 사내 실무 미팅(clovanote 녹취록, `음성 260810_133830.txt`)에서 상무님이 이 에이전트를 직접 테스트하며 지적한 내용 3가지를 그대로 처리함.
- **1) 신뢰도 버그 수정.** "존재하지 않는 엉뚱한 주소를 쳐도 신뢰도가 90%로 나온다"는 지적. 원인: `_geocode()`가 정확한 도로명주소 검색에 실패하면 카카오 키워드/장소 검색(`keyword_geocode`)으로 폴백하는데, 이건 느슨한 프리텍스트 매칭이라 존재하지 않는 주소도 비슷한 이름의 엉뚱한 장소에 매칭될 수 있음. 그런데 `calculate_confidence()`는 위치가 정확 매칭인지 느슨한 매칭인지 구분하지 않고 "위치가 있다"는 사실만으로 가점을 줬음. `market_agent/models.py`의 `GeoPoint.source`가 이미 `"kakao"`(정확) vs `"kakao-keyword"`(느슨) 를 구분하고 있어서, `rule_engine.calculate_confidence()`에서 `source`에 따라 위치 가점을 0.18 → 0.04로 차등 적용하도록 수정. 추가로 `agent.py._geocode()`가 키워드 폴백을 쓸 때 `limitations`에 "정확한 주소로 찾지 못해 추정한 위치"라는 경고 문구(추정 주소 포함)를 남기도록 변경. 회귀 테스트 2개 추가 (`tests/test_rule_engine.py`, 신규 `tests/test_agent_geocode.py`).
- **2) 전세가율에 추세(방향성) 표시 추가.** "그냥 63.2%다 이래만 나와 있고, 최근에 올라갔다 내려갔다 이런 말이 없다"는 지적. 매매 실거래가 카드는 이미 "직전 3개월 대비 +x%"로 방향을 보여주고 있었는데 전세가율 카드만 스냅샷 숫자뿐이었음. `market_agent/collectors/molit_rent.py`: `build_jeonse_evidence()`에 `baseline_trade_records`/`baseline_rent_records` 선택 인자를 추가해, 직전 3개월 전세가율을 같이 계산할 수 있으면 제목에 "직전 3개월 대비 X%p" 방향을 표시 (표본 부족하면 기존처럼 스냅샷만 표시, 하위호환). `JeonseRatioCollector.collect()`가 `molit.py`의 매매 실거래가 수집기와 동일한 패턴으로 baseline(직전 3개월, offset=3) 데이터도 같이 조회하도록 변경. 회귀 테스트 3개 추가.
- **3) 신규 입주 물량 반영 — 조사만 완료, 구현은 다음 세션.** "입주 물량이 공급 발표보다 집값에 더 직접적"이라는 의견 (아실 앱 참고 제안 있었으나 아실은 비공식 서비스라 대상 아님). 조사 결과 `한국부동산원_주택공급정보_입주예정물량정보`(data.go.kr, ID 15111714)라는 공식 출처를 찾음 — 미분양 때와 달리 전국 통일 출처가 있다는 점은 좋은 소식. 다만 (a) 갱신주기가 반기(6개월)로 국토부 실거래가(월 단위)보다 훨씬 성기고, (b) `openapi.do` 문서 페이지가 없어 필드 스펙이 미검증 상태. 매매 실거래가 때 문서와 실제 End Point/필드가 달라 버그 났던 전례가 있어서, 실제 활용신청 승인 화면으로 필드명을 확인하기 전까지는 코드를 작성하지 않기로 함. 자세한 내용은 `ROADMAP.md` 6순위 참고.
- 테스트 총 82개 통과 (기존 77개 + 오늘 5개: 신뢰도 2개 + 전세가율 추세 3개).
- **git 커밋 & 푸시 완료.** 커밋 `e6e131d`, `origin/main`에 반영, Render 배포 확인함 (Live 상태, 사용자가 직접 대시보드에서 확인).
- 참고: 폴더에 있는 `음성 260810_133830.txt`(미팅 녹취록 원문)는 git에 커밋하지 않음(untracked 상태 유지) — 사내 미팅 내용이라 저장소에 올릴 이유가 없음.

**2026-08-10 (2차): 배포 후 실사용 재현 테스트로 신뢰도 버그·"상승동력" 오분류 버그 추가 발견 및 수정**

- 사용자가 배포된 서비스에서 실제로 "군포로 111"(존재하지 않는 주소)을 입력해 캡처를 보내줌. 결과: `분석 대상: 군포로 111`인데 `신뢰도: 90%`로 여전히 높게 표시됨. 화면 하단에는 카카오가 "군포로 109"로 느슨하게 추정했다는 안내는 잘 떴음 (1차 수정의 `limitations` 부분은 정상 동작).
- **신뢰도 버그 2차 원인**: 1차 수정에서 위치 가점만 0.18 → 0.04로 낮췄는데, 이 케이스는 근거가 28건(정책 5+뉴스+생활인프라+실거래가)이나 모여서 카테고리 가점 합계만으로 이미 0.9 상한에 도달해버림 — 위치 가점을 아무리 깎아도 결과가 상한에서 그대로 잘려서 90%로 보였음. **수정**: `calculate_confidence()`에서 위치가 `kakao-keyword`(느슨한 매칭)일 때는 개별 가점이 아니라 **신뢰도 전체의 상한선을 0.9 → 0.55로 고정**하도록 변경. 근거가 아무리 많이 쌓여도 위치 자체가 불확실하면 신뢰도가 그 이상 못 올라감. 캡처와 동일한 조건(근거 28건, 느슨한 매칭)을 그대로 재현하는 회귀 테스트 추가.
- **"상승동력" 카드 오분류 버그 발견**: 같은 캡처에서 "상승동력"과 "생활권" 카드가 완전히 똑같은 텍스트("반경 3km 내 학교 15곳 탐지: 둔대초등학교...")를 보여주고 있었음. 원인 확인 결과, 이건 우연이 아니라 `market_agent/templates/index.html`의 "상승동력" 카드가 진짜 호재성 뉴스(`good_news`)가 없을 때 `local_factors`(생활 인프라) 데이터를 그대로 재활용해서 보여주는 **의도된 폴백 로직**이었음 (`{% elif report.local_factors %}`). 이게 바로 2026-08-10 미팅에서 상무님이 지적했던 문제의 실체였음 — 상무님이 회의 중 읽었던 "생활 인프라는 강하지만... 대형마트나 이런 거 고르게 있고... 기본 수요가 탄탄하다"는 문구가 정확히 이 `local_factors` 문구였고, 그게 "상승동력"이라는 이름을 달고 나왔던 것. 생활 인프라(학교·마트 등)는 이미 현재 시세에 반영된 기존 기반이지 "향후 상승 요인"이 아니므로, 이 둘을 섞어서 보여주면 안 됨.
- **수정**: `index.html`의 "상승동력" 카드에서 `local_factors` 폴백 분기를 완전히 제거. `good_news`가 없으면 정직하게 "뚜렷한 호재 없음"과 "생활 인프라는 상승동력이 아니라 이미 반영된 기반 수요"라는 설명을 보여주도록 변경. `market_agent/analysis/openai_analyzer.py`의 OpenAI 프롬프트에도 같은 원칙(생활 인프라 ≠ 상승동력, good_news 비어 있으면 생활 인프라로 대체하지 말고 "확인된 변화 신호 없음"이라고 명시)을 추가.
- 템플릿/프롬프트 변경이라 별도 단위 테스트는 없음 (이 프로젝트는 템플릿 렌더링을 자동 테스트하지 않음 — 다음에 배포 후 실제 화면으로 재확인 필요).
- 테스트 총 83개 통과 (신뢰도 2차 회귀 테스트 1개 추가).
- 신뢰도 2차 수정은 커밋 `4ea7d8d`로 이미 반영됨. 상승동력 카드/OpenAI 프롬프트 수정은 이 노트와 함께 커밋 예정.

- 사용자가 실제 배포 화면에서 신뢰도 55% 표시, "상승동력" 카드의 "뚜렷한 호재 없음" 문구를 육안으로 확인함 — 둘 다 정상 작동 확인됨.

**2026-08-10 (3차): 미래 신호(재건축 이주·시설 이전·교통망) 수집 로직 자체 개선**

- 2차까지는 "이미 수집된 호재 신호를 생활 인프라와 섞어서 잘못 보여주던 버그"를 고친 것이고, "호재 신호 자체를 더 잘 찾아내는" 수집 로직 개선은 남아있었음. 이번 3차에서 착수.
- `market_agent/keywords.py`: `POSITIVE_KEYWORDS`에 "이주", "이전", "예타"(예비타당성조사), "환승센터", "신설" 추가. "이주"는 기존 `REDEVELOPMENT_STAGE_WEIGHTS`에도 있었지만 그 가중치는 재건축/재개발/정비사업/정비구역 태그가 이미 있어야만 적용되는 구조라, 다른 정비사업 키워드 없이 "이주"만 언급된 기사는 그동안 놓치고 있었음.
- `market_agent/collectors/naver.py`:
  - `GOOD_QUERIES`에 `"{target} 이주 이전 신설"` 쿼리 추가 — 상무님이 예로 든 시나리오(재건축 이주로 전세가 급등, 운전면허시험장 같은 대형 시설 이전으로 생기는 부지 개발)를 직접 겨냥.
  - `classify_source_domain()`/`apply_source_domain_reliability()` 신규 추가: 출처 URL이 정부 공식 도메인(`.go.kr`)이면 신뢰도 가점(`공식출처` 태그), 블로그/카페/유튜브 등 개인 콘텐츠 플랫폼이면 신뢰도 감점(`블로그출처` 태그). "블로그 카더라보다는 믿을 만한 것만 가지고 예측해야 한다"는 지적을 그대로 반영. `collect()`에서 기존 `apply_region_relevance`와 같은 자리에서 모든 근거에 적용.
- 회귀 테스트 6개 추가 (`test_keywords.py` 2개, `test_naver_collector.py` 4개). 전체 89개 통과.
- README.md에 이 동작 한 줄 반영.
- 아직 git 커밋/푸시 전.

**아직 안 끝난 것 (2026-08-10 기준)**

1. 신규 입주 물량 API 활용신청 진행 여부 확인 (ROADMAP.md 6순위).
2. 오늘 추가한 새 키워드/쿼리/도메인 신뢰도 로직은 실제 네이버 API 키로 검증하지 않음 (유닛 테스트는 Fake 클라이언트 기준). 다음에 실사용 화면에서 "이주"/"이전"/`.go.kr` 출처 근거가 실제로 잘 잡히는지, 새 쿼리가 노이즈(무관한 이주/이전 뉴스)를 과도하게 끌어오지는 않는지 확인 필요.

## 지난 세션 작업 기록 (2026-07-10 기준)

**2026-07-10: `/screen` Render 배포 후 "계속 로딩만 됨" 버그 수정 (병렬 조회)**

- 사용자가 Render 배포 주소에서 `/screen`을 열었더니 응답 없이 계속 로딩만 됨.
- **원인**: `screen_districts()`가 서울 25개 구를 순차(for loop)로 돌면서, 구 하나당 국토부 실거래 API를 최대 6~9번(최근 3개월+직전 3개월 매매, 전세 3개월) 순차 호출함. 전체로 보면 요청 1번에 최대 150~225번의 네트워크 왕복이 직렬로 쌓이는 구조라, 로컬(빠른 네트워크)에서는 안 드러났지만 Render 배포 환경에서는 응답까지 몇 분씩 걸리거나 타임아웃 나는 문제였음. (로컬 테스트는 전부 Fake 클라이언트라 실제 네트워크 지연이 없어서 이 문제를 놓쳤음 — 실사용 환경에서만 드러난 버그.)
- **수정**: `market_agent/screener.py`의 `screen_districts()`에서 구 단위 조회를 `concurrent.futures.ThreadPoolExecutor`(`MAX_PARALLEL_DISTRICTS = 8`)로 병렬 처리하도록 변경. 구 25개가 서로 완전히 독립적인 조회라 스레드로 동시에 돌려도 안전함. 인위적으로 네트워크 지연(0.2초)을 흉내낸 시뮬레이션에서 25구 순차 약 30초 → 병렬 처리 후 약 4.8초로 확인 (약 6배 단축).
- 테스트는 기존 77개 그대로 통과 (Fake 클라이언트는 스레드 세이프, 순서 의존 로직 없음 확인).
- **git 커밋 & 푸시 완료.** 커밋 `7cf5fe7` (2개 파일 변경), `origin/main`에 반영됨. Render 자동 배포 트리거됨.
- **⚠️ 다음 세션 시작 시 최우선 확인할 것**: 사용자가 아직 배포 후 실제로 `/screen`이 정상 로딩되는지 확인 전에 다른 작업으로 넘어감. 다음에 이어갈 때 제일 먼저 "지역 스크리닝 페이지 이제 잘 열려?" 물어보고, 여전히 느리거나 타임아웃 나면 병렬 워커 수 조정(`MAX_PARALLEL_DISTRICTS`)이나 조회 기간 축소(3+3개월 → 2+2개월) 등 추가 튜닝 필요.

## 지난 작업 기록 (2026-07-09 기준)
