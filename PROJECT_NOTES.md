...
## 다음 세션 시작 가이드 (2026-08-10 기준 — "다음 작업 하자"고 하면 이것부터)

**오늘 상태**: 회의록 지적사항 4개 중 3개(신뢰도 버그, 전세가율 추세, 상승동력/생활인프라 혼동) 완료 + 배포 + 실사용 검증까지 끝남. 남은 1개(신규 입주 물량)는 데이터 소스 조사 중, 코드 작업은 아직 시작 안 함. **git은 전부 커밋/푸시 완료, 작업 트리 깨끗함** (마지막 커밋 `d710d64`). 테스트 89개 전부 통과 상태.

**바로 이어서 할 일 (우선순위 순)**:
1. **입주 물량 API 활용신청** — data.go.kr에서 `한국부동산원_청약홈 분양정보 조회 서비스`(ID 15098547) 활용신청 (5순위 청약 경쟁률 API `ID 15098905`와 같은 계정으로 같이 신청 추천). 승인되면 기술문서(`기술문서_청약홈 분양정보 조회 서비스_260129.docx`)를 다운로드해서 **"입주예정월"/"총세대수" 필드가 실제로 있는지부터 확인** — 없으면 이 후보도 무산이니 다른 대체 소스를 또 찾아야 함. (자세한 배경: 아래 5차 기록, `ROADMAP.md` 6순위)
2. 필드 확인되면 코드 작업 시작: `market_agent/collectors/` 아래 새 수집기 작성 (기존 `molit.py`/`molit_rent.py` 패턴 재사용), `agent.py`에 연동, 테스트 작성.
3. 여유 있으면 `ROADMAP.md` 5순위(청약 경쟁률)도 같이 진행 — 같은 API 계열이라 시너지 있음.
4. 오늘 3차에서 추가한 새 키워드("이주"/"이전"/"예타" 등)와 도메인 신뢰도 로직(`.go.kr` 가점/블로그 감점)은 아직 실제 네이버 API로 검증 안 됨 — 실사용 화면에서 잘 잡히는지, 노이즈는 없는지 한 번 확인해보면 좋음 (급하진 않음).
...
- **git 커밋 & 푸시 완료.** 커밋 `9e08985`, `origin/main`에 반영, Render 자동 배포 트리거됨.

**2026-08-10 (4차): 아실 화면 캡처 대조 후 입주물량 데이터셋 재조사 — 예상보다 데이터 수준 좋음**

- 사용자가 회의록에서 언급된 "아실"(asil.kr) 앱의 "입주물량" 화면 캡처를 공유함. 위치별 단지명/입주년월/세대수 표, 적정수요선 대비 연도별 막대그래프, 지도 위 공급 위치 핀으로 구성.
- 아실 자체는 여전히 비공식 서비스라 연동 대상 아님(1차 조사 때 판단 유지). 대신 이 캡처를 기준으로 `한국부동산원_주택공급정보_입주예정물량정보`(ROADMAP 6순위) 페이지를 다시 상세 조회.
- **좋은 소식**: 이 데이터셋이 시/군/구 집계치가 아니라 **단지 수준 상세 데이터**였음. 문서상 확인된 항목: `입주예정월`(YYYYMM), `지역`(2자리 코드), `사업유형`, `주소`, `아파트명`, `세대수`. 기준시점 2025-12-31, 2026-01~2027-12(향후 2년) 범위, 30세대 이상 공동주택 기준 675개 레코드. "주택건설 실적정보 + 입주자모집공고 + 정비사업 추진실적을 종합해 지자체 검증까지 거침" — 아실의 민간 출처("분양물량조사")보다 오히려 공신력 높음.
- ⚠️ 이 항목명은 data.go.kr 페이지 설명 요약이지 실제 다운로드한 원본 CSV 헤더가 아님. `지역`이 시/도 2자리 코드로 추정되는데 시/군/구까지 좁히려면 `주소` 텍스트 매칭이 필요할 수 있음(`molit.py`의 `filter_relevant()` 패턴 재사용 가능). 위경도가 없어 반경 필터링 대신 구/동 이름 매칭으로 대체 예상.
- 사용자에게 data.go.kr 활용신청 절차 안내함 (기존 MOLIT/전월세 API 신청 때와 동일한 계정/절차).
- **바로 이어서 확인한 결과: 실제로는 "공공기관 전용" 데이터셋이었음.** 사용자가 data.go.kr에서 "API 활용신청" 버튼을 눌러보니 공공기관만 신청 가능하다는 안내 팝업이 떴음. 필드 스펙이 좋아 보였던 것과 무관하게, 애초에 민간 계정으로는 활용신청 자체가 불가능한 카테고리라 **이 데이터셋은 완전히 이용 불가로 최종 보류**. ROADMAP.md 6순위를 "⏸️ 보류 (공공기관 전용이라 이용 불가)"로 갱신함.
- ROADMAP.md 6순위에 상세 내용 반영, 코드 변경 없음(문서만).

**2026-08-10 (5차): 대체 데이터 소스 재검색 — 청약홈 분양정보 API 후보 발견**

- 사용자 요청으로 "공공기관 전용이 아닌 다른 입주예정 물량 확인 방법"을 재검색.
- **한국부동산원_청약홈 분양정보 조회 서비스**(data.go.kr, ID 15098547)를 유력 후보로 확인. "이용허락범위: 제한 없음"이고 "개인 서비스키" 신청 옵션이 있어 공공기관 전용이 아님. ROADMAP 5순위(청약 경쟁률, ID 15098905)와 같은 "청약홈" API 계열이라 같은 계정으로 같이 신청 가능할 가능성 높음.
- 최근(2026-01-29) 기술문서 업데이트 내역에서 "주택명", "공급위치" 필드 추가가 명시된 것을 확인. 다만 "입주예정월"/"총세대수" 필드 존재 여부는 기술문서(.docx)를 직접 못 열어봐서 미확인 — 활용신청 후 이 문서로 먼저 확인 필요.
- ROADMAP.md 6순위에 대체 후보로 반영, 코드 변경 없음(문서만).

**아직 안 끝난 것 (2026-08-10 기준)**

1. 신규 입주 물량: 1차 후보(한국부동산원_입주예정물량정보)는 공공기관 전용이라 무산. 대체 후보(한국부동산원_청약홈 분양정보 조회 서비스, ID 15098547)를 찾음 — 다음 세션에서 활용신청 후 기술문서로 "입주예정월"/"총세대수" 필드 존재 여부부터 확인 (ROADMAP.md 6순위).
2. 오늘 추가한 새 키워드/쿼리/도메인 신뢰도 로직(3차)은 실제 네이버 API 키로 검증하지 않음 (유닛 테스트는 Fake 클라이언트 기준). 다음에 실사용 화면에서 "이주"/"이전"/`.go.kr` 출처 근거가 실제로 잘 잡히는지, 새 쿼리가 노이즈(무관한 이주/이전 뉴스)를 과도하게 끌어오지는 않는지 확인 필요.
