...
1. 배포 후 실제 화면에서 "상승동력" 카드가 더 이상 생활 인프라 문구를 재활용하지 않고 "뚜렷한 호재 없음"으로 정직하게 나오는지 육안 확인 필요 (템플릿은 자동 테스트가 없어서 실제 화면 확인이 유일한 검증 수단).
2. 회의록 4개 지적 중 유일하게 남은 항목: 재건축 이주 계획·교통망 발표 같은 "미래 변화" 신호를 뉴스에서 더 잘 골라내는 것 자체는 아직 손대지 않음 (오늘 고친 건 "이미 있는 호재 신호를 생활 인프라와 섞어서 보여주던 표시 버그"이지, "호재 신호 자체를 더 잘 찾아내는" 수집 로직 개선은 아님).
3. 신규 입주 물량 API 활용신청 진행 여부 확인 (ROADMAP.md 6순위).
